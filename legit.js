...
class Core {
        ...
        window.fetch = async function(url, options) {
            ...
            try {
                ...
                if (data?.data?.assessmentItem?.item?.itemData) {
                    ...
                    if (APP.cfg.spoofCheck && itemData.question?.widgets) {
                        ...
                        sendToast("✔️ Resposta correta marcada!", 2000);
                ...
                console.error("Erro no mod:", error);
            }
    ...
    static async setupAuto() {
        ...
        function clickAnswerOption() {
            const radioInputs = document.querySelectorAll('input[type="radio"]');
            for (const radio of radioInputs) {
                if (!radio.checked && radio.offsetParent !== null) {
                    radio.click();
                    return true;
                }
            }
            ...
            const clickableAnswers = document.querySelectorAll('[role="radio"], [role="checkbox"], .answer-option, [class*="choice"], [class*="option"]');
            for (const answer of clickableAnswers) {
                if (answer.offsetParent !== null && !answer.getAttribute('aria-checked')) {
                    answer.click();
                    return true;
                }
            }
        ...
        while (true) {
            try {
                if (APP.cfg.autoCheck) {
                    const answerClicked = clickAnswerOption();
                    
                    if (answerClicked) {
                        await delay(APP.cfg.speed / 2);
                    }
