    ...
    static init() {
        ...
        panel.innerHTML = `
            ...
            <div class="crimson-header">
                <img src="https://onecompiler.com/favicon.ico" class="crimson-logo" alt="Logo">
            ...
            <div class="crimson-content">
                ...
                <div class="crimson-credit">Made by [@Crimsonstrauss]</div>
            </div>
        `;
        ...
        speedSlider.onchange = () => {
            const value = parseInt(speedSlider.value);
            const speed = APP.cfg.speedOptions[value];
            sendToast("⏱️ Velocidade: " + speed + "ms", 2000);
        };
        ...
        document.getElementById("darkModeCheck").onchange = (e) => {
            ...
            if (typeof DarkReader !== "undefined") {
                if (APP.cfg.darkMode) {
                    DarkReader.enable();
                    sendToast("🌑 Dark Mode Enabled", 3000);
                } else {
                    DarkReader.disable();
                    sendToast("☀️ Dark Mode Disabled", 3000);
                }
            } else {
...
class Core {
    ...
    static async loadExternalLibraries() {
        try {
            ...
            if (typeof Toastify !== "undefined") {
                sendToast("☪️ Crimson carregado com sucesso!", 2000);
            } else {
            ...
            try {
                ...
                if (data?.data?.assessmentItem?.item?.itemData) {
                    ...
                    if (itemData.question.content[0] === itemData.question.content[0].toUpperCase() && APP.cfg.spoofCheck) {
                        ...
                        sendToast("🔓 Question Bypassed", 2000);
    ...
    static async setupAuto() {
        ...
        function clickButton(className) {
            ...
            if (button) {
                ...
                if (button.textContent === "Mostrar resumo") {
                    sendToast("Exercício completo!", 2000);
        ...
        // Loop infinito para auto-completar
        while (true) {
            await autoComplete();
...
class KeyboardShortcuts {
    static init() {
        document.addEventListener("keydown", (e) => {
            ...
            // Tecla Q - Toggle Menu (maiúscula ou minúscula)
            if (e.key === "q" || e.key === "Q") {
                e.preventDefault();
                ...
                if (panel) {
                    if (panel.style.display === "none") {
                        ...
                        sendToast("👁️ Menu visível", 1500);
                    } else {
                        panel.style.display = "none";
                        sendToast("🙈 Menu oculto", 1500);
                    }
                }
            }
            
            // Tecla R - Toggle Notificações (maiúscula ou minúscula)
            if (e.key === "r" || e.key === "R") {
                e.preventDefault();
                
                if (notificationsEnabled) {
                    // Mostra última notificação antes de desativar
                    sendToast("🔕 Notificações desativadas", 2000);
                    // Aguarda um pouco para mostrar a notificação
                    setTimeout(() => {
                        notificationsEnabled = false;
                    }, 100);
                } else {
                    // Ativa as notificações primeiro
                    notificationsEnabled = true;
                    sendToast("🔔 Notificações ativadas", 2000);
                }
            }
        });
